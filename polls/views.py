...
class TableView(View):
    """
        ...
        method diplaying
    """
    TEMPLATE_NAME = 'polls/preprocess1.html'
...
def data_view(request):
    return render(request, 'polls/logic/clustering_data_view.html')


def cf_data_view(request):
    return render(request, 'polls/logic/classification_data_view.html')


def labeled(request):
    return render(request, 'polls/labeled.html')


def unlabeled(request):
    return render(request, 'polls/unlabeled.html')


def iris_data_set(request):
    return render(request, 'polls/iris_data_set.html')


def car_data_set(request):
    return render(request,"polls/car_data_set.html")
...
def handle_uploaded_file(f):
    ...
    # print(file_obj.name)
    # print(f)
    with open('polls\\data\\name.csv', 'wb+') as destination:
...
strategies_list=[  ['pihata','LogisticRegression'],
                    ['picard', 'KNN'],
                    ['hatanalizi', 'DecisionTree'],
                    # ['sisalgo1', 'GradientBoosting'],
                    ['nonsisalgo1', 'SVC'],
                    ['nonsisalgo2', 'RandomForest'],
                    ['nonsisalgo3', 'MLP'],
                    ['de', 'GaussianNB'],
                ]
...
strategies = [  ['分类',strategies_list],]

simulations=[  ['聚类',simulations_list],]

calculations=[  ['聚类',calculations_list],]
...
def index(request):
    ...
    return  render(request, 'polls/index1.html', {'iletisims':iletisims, 'simulations':simulations , 'calculations':calculations, 'calculations_list':calculations_list, 'strategies': strategies, 'simulations_list':simulations_list, 'strategies_list': strategies_list})
...
class Cf_name(View):
    ...
    TEMPLATE_NAME = 'polls/preprocess1.html'
    form_inital = {"drop_missing": True, "char_to_digit": True, "Classifier": "CF"}
    # form_inital = {"Classifier": "CF"}

    root_path = get_root_path()

    # param is table name
    def get(self, request, cf_name):
        # csv_name = cf_name +".csv"
        # print(csv_name)
        # dfmodel = get_object_or_404(DataFrameModel, df_description=slug)
        # df = read_csv_file(self.root_path + DATADIRPATH + csv_name)
        form = PreprocessForm1(initial=self.form_inital)
        context ={
            "form": form,
            "tableName": cf_name,
            # "table": df_to_html(df)
    ...
    # # submit form
    def post(self, request, cf_name):
        # form = PreprocessForm1(request.POST)
        # #
        # # form = PreprocessForm(initial=self.form_inital)
        # if form.is_valid():
        #
        ...
        # csv_name1 = form.cleaned_data['method_selection']
        #
        # csv_name = csv_name1 + ".csv"
        # # dfmodel = get_object_or_404(DataFrameModel, df_description=slug)
        # form = PreprocessForm(request.POST)
        form = PreprocessForm1(request.POST)
        #
        ...
        if form.is_valid():
            ...
            # save new df
            new_csv_description = csv_name1 + "-after-preprocessing"
            new_csv_store_name =csv_name.replace(".csv", "_pre.csv")
            ...
            print(form.cleaned_data['method_selection'])
             ...
             # redirect to specific method
            return redirect("/polls/" + "cf/" + cf_name + "/" + new_csv_store_name.replace(".csv","") + "/")
...
"""
    Section:
        Classification and showing its result
"""
class ClassificationView1(View):
    """
        This view is used for classification parameters seting
    """
    TEMPLATE_NAME = 'polls/logic/classification1.html'
    root_path = get_root_path()
    # form_inital = {"Classifier": "KNN", "train_ratio" : "0.7"}


    def get(self, request, cf_m, new_csv_store_name):
        # dfmodel = get_object_or_404(DataFrameModel, df_description=new_csv_store_name)
        df = read_csv_file(self.root_path + TMPDIRPATH + new_csv_store_name+".csv")
        print(cf_m)

        form_inital = {"Classifier": cf_m, "train_ratio": "0.7"}
        print(cf_m)

        form = ClassificationForm1(initial=form_inital)
        context = {
            "form": form,
            "tableName": new_csv_store_name,
            "table": df_to_html(df)
    ...
    # rendering post and redirect to showing result
    def post(self, request, cf_m, new_csv_store_name):
        # dfmodel = get_object_or_404(DataFrameModel, df_description=new_csv_store_name)
        df = read_csv_file(self.root_path + TMPDIRPATH + new_csv_store_name+".csv")
        form = ClassificationForm1(request.POST)
        if form.is_valid():
            print(form.cleaned_data["classification_parameters"])
            # param = form.cleaned_data
            new_csv, train_stat = MyClassification(df, form.cleaned_data["label_name"], form.cleaned_data["Classifier"],form.cleaned_data["train_ratio"], param=form.cleaned_data["classification_parameters"])
...
class Cr_name(View):
    ...
    def get(self, request, cf_name):
        ...
        # dfmodel = get_object_or_404(DataFrameModel, df_description=slug)
        # df = read_csv_file(self.root_path + DATADIRPATH + csv_name)
        form = PreprocessForm2(initial=self.form_inital)
        context ={
            "form": form,
            "tableName": cf_name,
            # "table": df_to_html(df)
    ...
    def post(self, request, cf_name):
        ...
        # # dfmodel = get_object_or_404(DataFrameModel, df_description=slug)
        # form = PreprocessForm(request.POST)
        form = PreprocessForm2(request.POST)
        #
        # form = PreprocessForm(initial=self.form_inital)
        if form.is_valid():
            print(form.cleaned_data['method_selection'])
        ...
        if form.is_valid():
            ...
            print(form.cleaned_data['method_selection'])
             ...
             # redirect to specific method
            return redirect("/polls/" + "cr/" + cf_name + "/" + new_csv_store_name.replace(".csv","") + "/")
...
class ClusteringView1(View):
    TEMPLATE_NAME = 'polls/logic/clusting.html'
    root_path = get_root_path()
    # form_inital = {"ClusterAlgo": "KMeans"}


    def get(self, request, cr_m, new_csv_store_name):

        df = read_csv_file(self.root_path + TMPDIRPATH + new_csv_store_name + ".csv")

        form_inital = {"Cluster_Algo": cr_m}
        # print(cr_m)
        # print("###########")
        form = ClusteringForm1(initial=form_inital)
        # {"Classifier": cf_m, "train_ratio": "0.7"}
        # print(form.Clustering_Parameters)
    ...
    # rendering post and redirect to showing result
    def post(self, request, cr_m, new_csv_store_name):

        df = read_csv_file(self.root_path + TMPDIRPATH + new_csv_store_name + ".csv")
        form = ClusteringForm1(request.POST)
        if form.is_valid():
            print(form.cleaned_data["Cluster_Algo"])
            # param = form.cleaned_data
            new_csv, train_stat = MyClustering(df, form.cleaned_data["Cluster_Algo"],
...
class Ae_name(View):
    ...
    def get(self, request, cf_name):
        ...
        # dfmodel = get_object_or_404(DataFrameModel, df_description=slug)
        # df = read_csv_file(self.root_path + DATADIRPATH + csv_name)
        form = PreprocessForm3(initial=self.form_inital)
        context ={
            "form": form,
            "tableName": cf_name,
            # "table": df_to_html(df)
    ...
    def post(self, request, cf_name):
        ...
        # # dfmodel = get_object_or_404(DataFrameModel, df_description=slug)
        # form = PreprocessForm(request.POST)
        form = PreprocessForm3(request.POST)
        #
        # form = PreprocessForm(initial=self.form_inital)
        if form.is_valid():
            print(form.cleaned_data['method_selection'])
        ...
        if form.is_valid():
            ...
            print(form.cleaned_data['method_selection'])
             ...
             # redirect to specific method
            return redirect("/polls/" + "re/" + cf_name + "/" + new_csv_store_name.replace(".csv","") + "/")
...
class AssociationRuleView1(View):
    TEMPLATE_NAME = 'polls/logic/association_rules.html'
    root_path = get_root_path()


    def get(self, request,re_m, new_csv_store_name):
        df = read_csv_file(self.root_path + TMPDIRPATH + new_csv_store_name + ".csv")
        # print(df)
        frequent_itemsets,rules = do_association_rule(df)
        print(type(frequent_itemsets))
...
def login_view(request):
    ...
    return render(request, 'polls/login.html', {"form": form, 'title': 'Login','iletisims':iletisims,'calculations':calculations,'calculations_list':calculations_list, 'simulations':simulations , 'strategies': strategies,'simulations_list':simulations_list, 'strategies_list': strategies_list})
...
def register_view(request):
    ...
    return render(request, 'polls/register.html', { 'title': 'Register','iletisims':iletisims,'calculations':calculations,'calculations_list':calculations_list, 'simulations':simulations , 'strategies': strategies,'simulations_list':simulations_list, 'strategies_list': strategies_list})
